...
export default class gameScene3 extends Phaser.Scene {
    ...
    create() {
        ...
        this.lives = 3;
        this.scoreText = this.add.text(325,235,'Crystals:' + this.score + '/53', {fontSize: '15px'});
        this.livesText = this.add.text(325,260,'Lives Left: ' + this.lives,{fontSize: '15px'});
        ...
        const heroDeath = {
            key: 'deathAnim',
            frames: this.anims.generateFrameNumbers('hero', {start: 12, end: 16}),
            framerate: 0.05
        }
        ...
        if (this.spaceKey.isDown && this.hero.body.blocked.down)
    {
                this.jumpSound.play();
        ...
        //char data
        this.hero = this.physics.add.sprite(5622,1530, 'hero', 0);
    ...
    trapSpike(hero,spik){
        if (this.lives > 0){
            ...
            console.log("active");
            hero.setPosition(5622,1530);
            hero.anims.play('respawnAnim');
            hero.on('animationcomplete', () => {hero.anims.play('idleLeft'),this.cursors.right.enabled = true,
    ...
    finishOrb(hero,fin){
        ...
        this.scene.start('endScene3', {score: this.score});
        return false;
    ...
    collectCryst(hero,crys){
        ...
        this.score += 1;
        this.scoreText.setText('Crystals: ' + this.score + '/53');
        return false;
    }
